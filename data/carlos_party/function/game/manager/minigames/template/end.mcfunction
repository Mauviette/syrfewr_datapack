...
tag @a[tag=cp.template,distance=..200] add cp.winner
clear @a[tag=cp.template,distance=..200]
tag @a[tag=cp.template,distance=..200] remove cp.template
...
execute as @a[distance=..200] at @s run playsound syrfewr:music_end_template record @s ~ ~ ~ 1 1
