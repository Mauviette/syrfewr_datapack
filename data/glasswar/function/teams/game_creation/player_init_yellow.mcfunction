...
tellraw @s [{"text":"La partie a demarré !","color":"green"},{"text":" Vous êtes dans l'équipe ","color":"white"},{"text":"jaune","color":"yellow"},{"text":"!","color":"white"}]
tp @s ~ ~ ~ facing entity @e[limit=1,sort=nearest,tag=gw.game_manager_init]
scoreboard players enable @s leave
tag @s add gw.playing_yellow
tag @s add gw.playing_teams
tag @s remove gw.starting_teams
tag @s add gw.change_glass
tag @s add s.playing
tag @s remove gw.chosing_team