...
#Desc
$data modify storage tc.ui_tower mask[1].components.lore set value ['{"text":"Niveau $(level)","color":"green","italic":false}','{"text":""}','{"text":"$(damage) dégats par tir","color":"red","italic":false}','{"text":"$(range) blocs de portée","color":"dark_green","italic":false}','{"text":"$(fire_rate) tirs par seconde","color":"aqua","italic":false}']
tellraw @a {"score": {"name": "#level", "objective": "s.temp"}}
