...
execute if items entity @s weapon.* *[custom_data~{wrench:1}] if entity @e[tag=tc.tower,distance=..200] run function towercraft:game/player/wrench/ui/player/teleport
execute unless items entity @s weapon.* *[custom_data~{wrench:1}] if score @s tc.ui_id matches 0.. run function towercraft:game/player/wrench/ui/player/close
execute if items entity @s weapon.* *[custom_data~{wrench:1}] unless entity @e[tag=tc.tower,distance=..200] if score @s tc.ui_id matches 0.. run function towercraft:game/player/wrench/ui/player/close
