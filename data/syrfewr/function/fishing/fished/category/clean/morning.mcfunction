...
##Poissons
execute if score #random_type s.temp matches 1..69 unless score @p[scores={s.hasFished=1..}] s.fishingRodType matches 2 store result score #random_2 s.temp run random value 1..100
execute if score #random_type s.temp matches 1..69 if score @p[scores={s.hasFished=1..}] s.fishingRodType matches 2 store result score #random_2 s.temp run random value 100..125
...
#Poisson rouge
execute if score #random_2 s.temp matches 101..125 if score @p[scores={s.hasFished=1..}] s.fishingRodType matches 2 run data modify storage s.fished id set value 2
execute if score #random_2 s.temp matches 101..125 if score @p[scores={s.hasFished=1..}] s.fishingRodType matches 2 run data modify storage s.fished fish set from storage s.fish category.clean[2].id
