...
##Poissons
execute if score #random_type s.temp matches 1..70 unless score @p[scores={s.hasFished=1..}] s.fishingRodType matches 1 store result score #random_2 s.temp run random value 1..70
execute if score #random_type s.temp matches 1..70 if score @p[scores={s.hasFished=1..}] s.fishingRodType matches 1 store result score #random_2 s.temp run random value 1..90
tellraw @a {"score": {"name": "#random_2", "objective": "s.temp"}}
#Saumon
...
#Carpe
execute if score #random_2 s.temp matches 71..90 if score @p[scores={s.hasFished=1..}] s.fishingRodType matches 1 run data modify storage s.fished id set value 2
execute if score #random_2 s.temp matches 71..90 if score @p[scores={s.hasFished=1..}] s.fishingRodType matches 1 run data modify storage s.fished fish set from storage s.fish category.clear[2].id
