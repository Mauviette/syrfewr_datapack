...
execute if score #has_all_skins s.temp matches 1 run tellraw @s ["Vous avez trouvé un skin ",{"text":"légendaire","color":"yellow"},", mais vous les aviez déjà tous."]
...
#execute if score #id_skin s.temp matches 1 run function syrfewr:lootbox/trouvaille/skin/test_give {"type":"bows","skin":"shortbow",rarity:"legendary"}