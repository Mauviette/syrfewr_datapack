...
execute if score #trouvaille s.temp matches 1..33 run scoreboard players add @s s.creditsToAddBonus 80
execute if score #trouvaille s.temp matches 1..33 run tellraw @a[scores={s.seeSpecialOpening=1},distance=0.00001..] [{"selector": "@s"},{"text":" a obtenu "},{"text":"80 crédits","color":"gold"},"!"]
execute if score #trouvaille s.temp matches 1..33 run scoreboard players set #trouvaille_id s.temp 20

execute if score #trouvaille s.temp matches 34..59 run scoreboard players add @s s.creditsToAddBonus 150
execute if score #trouvaille s.temp matches 34..59 run tellraw @a[scores={s.seeSpecialOpening=1},distance=0.00001..] [{"selector": "@s"},{"text":" a obtenu "},{"text":"150 crédits","color":"gold"},"!"]
